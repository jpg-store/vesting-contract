...
parsePortionFromString s =
  case break (== ':') s of
    (ts, ':' : as) -> Portion <$> parseDeadline <*> parseAmount
      where
...
instance ParseRecord Portion where
  parseRecord =
    option parsePortionOpt . mconcat $
      [ long "portion"
...
instance ParseField Portion where
  parseField msg longName shortName _ =
    option parsePortionOpt $
...
writeDatum output beneficiaries schedule = write . plutusDataToJSON $ datum
  where
